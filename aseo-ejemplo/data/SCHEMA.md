...
## 18. `historico` (array, opcional · usado por Capa 3)

**Añadido en v2 (2026-05-08) para Capa 3 — Operativo Pyme.**
Array de meses anteriores. Alimenta el chart de tendencia del Looker Studio. Capa 1 (web demo) lo ignora; Capa 3 lo lee. Si no existe, el script `crear-cliente.py` de Capa 3 lo genera sintéticamente.

Cada item:

| Campo | Tipo | Notas |
|---|---|---|
| `mes` | string ISO | Formato `"YYYY-MM"`. Ej: `"2026-03"` |
| `ventas` | number | CLP totales del mes |
| `margen_pct` | number 0-1 | Margen consolidado |
| `utilidad` | number | CLP del mes |
| `punto_equilibrio` | number | CLP necesarios para cubrir fijos |
| `top_servicio` | string | Nombre del servicio #1 del mes (`"—"` si no aplica) |
| `notas` | string | Eventos relevantes del mes (ej: "promo activada"). Vacío `""` si nada |

Recomendado: 3-12 meses. Para demos: 3 meses (suficiente para mostrar tendencia sin saturar).

---

## 19. `forecast` (objeto, opcional · usado por Capa 3)

**Añadido en v2 (2026-05-08) para Capa 3 — Operativo Pyme.**
Proyección con 2 escenarios. Alimenta la vista "Tendencia y Forecast" del Looker.

| Campo | Tipo | Notas |
|---|---|---|
| `fecha_calculo` | string ISO | Cuándo se generó la proyección |
| `metodologia` | string | 1-2 frases que explican cómo se calculan los escenarios |
| `escenarios` | objeto | Contiene `a_mantener_ritmo` y `b_intervenir_palancas` |
| `delta_acumulado_6m` | objeto | `{ventas, utilidad, narrativa}` con la diferencia A vs B a 6 meses |

Cada escenario (`a_mantener_ritmo`, `b_intervenir_palancas`):

| Campo | Tipo | Notas |
|---|---|---|
| `label` | string | Nombre visible. Ej: `"Mantener ritmo actual"` |
| `descripcion` | string | 1 frase del concepto |
| `supuesto` | string | Marca `SUPUESTO:` con la asunción detrás del escenario |
| `meses` | array | 6 meses futuros con `{mes, ventas, margen_pct, utilidad}` |

Cada mes del array `meses`:

| Campo | Tipo |
|---|---|
| `mes` | string ISO `"YYYY-MM"` |
| `ventas` | number CLP |
| `margen_pct` | number 0-1 |
| `utilidad` | number CLP |

Recomendado: 6 meses adelante. Si supera 12 meses, la confianza cae rápido — marcar `RIESGO` explícito.
